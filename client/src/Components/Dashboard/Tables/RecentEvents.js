...
function RecentEvents(props) {
  ...
  const generalRecentSales = () => {
    ...
    const theRecentGeneralReceivings = pickupMachines.filter(
  ...
  const branchRecentSales = () => {
    const theRecentSales = Sales.filter(
      (filteredSale) => filteredSale.branchId == props.branchId
    );
    const theRecentReceivings = pickupMachines.filter(
      (filteredPickUp) => filteredPickUp.branchId == props.branchId
    );
                ...
                {recentSales.length > 0 &&
                  recentSales.map((sale, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-truncate" style={trnctMaxWidth}>
                        {sale.tradeName}
                      </td>
                      <td
                        className={`${
